/*=======================================================================
||				MsgHndlr.h	from 22May2020 //Marianna 27052020											||
||																		||
||======================================================================||
||																		||
||  PURPOSE:															||
...
//	short			PrSrcDocLocnRequired	[100];
	...
	// Marianna 27Mar2022
	short darkonai_max_hishtatfut_pct; 
...
typedef struct	// Trn. 6001 prescription-data structure - DonR 28Oct2014.
{
	...
	short				TotalUnitsPrescribed;	// DonR 18Jan2022 for Chanut Virtuality Trn. 6101.
...
typedef struct
{
	...
	int		old_ID;				// These two columns should really be dropped from table and code.
	short	old_ID_code;		// These two columns should really be dropped from table and code.
	int		BirthDate;			// DonR 08Aug2016: Added to support health-alert checking. date_of_bearth in DB (and new structure variable).
	short	age_months;
	short	current_vetek;
	short	prev_vetek;
	...
	char	house_num			[ 4 + 1];
	short	sex;
	int		date_of_bearth;		// Should really get a spelling fix in DB and code!
	...
	int		max_drug_date;
	int		old_member_id;		// These two columns should really be dropped from table and code.
	short	old_id_extension;	// These two columns should really be dropped from table and code.
...
typedef struct REST_service_control
{
	...
	char		*service_desc;		// varchar(100) in DB.
...
static int		REST_CurlMemoryClear (REST_MEMORY *MEM);
