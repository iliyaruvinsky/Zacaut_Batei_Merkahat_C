...
// DonR 20Mar2024 User Story #297879: This is a new version of the program, intended
...
int main (int argc, char *argv[])
{
    ...
    RETURN_ON_ERR (InitSonProcess (argv[0], DOC2AS400TOUNIX_TYPE, retrys, 0, DOCTOR_SYS));
...
// handler for message 3002 - insert to table LabResOther
// This one actually does something!
static int message_3002_handler (TConnection *p_connect, int data_len, bool do_insert)
{
	...
	if (Contr_N == 1)
	{
		...
		for (n_retries = 0; (n_retries < SQL_UPDATE_RETRIES) && (stop_retrying == false); n_retries++)
		{
			...
			{
				GerrLogMini (GerrId,"Trn. 3002: Error %d looking for lab result for member %d ReqID %d.",
			...
			if (v_count == 0)
			{
				...
				else
				{
					...
					if (SQLERR_error_test ())
					{
						GerrLogMini (GerrId,"Trn. 3002: Error %d adding lab result for member %d ReqID %d.",
										SQLCODE, LabResRec.MemberId.Number, LabResRec.ReqId);
...
// Note that the "real" 3008 is in As400UnixDocServer.c; this copy
// really should be just a "stub". However, I'm leaving this version
// "live", and I'll add a warning to the transactions-processed log
// message to indicate that 3008's shouldn't really be sent to
// As400UnixDoc2Server.
...
// handler for message 3001 - insert to table LABNOTES - no database updates.
static int message_3001_handler (TConnection *p_connect, int data_len, bool do_insert)
{
...
static int call_msg_handler (TSystemMsg msg, TConnection *p_connect, int len)
{
	...
	switch (msg)
	{
		...
		case 3002:	// Lab Results (from "normal" labs, not from moked)
					rows_received++;
					retcode = message_3002_handler (p_connect, len, ATTEMPT_INSERT_FIRST);
					break;

		case 3008:	// Hospitalization - shouldn't really be active in this program, but enabled anyway.
					rows_received++;
		...
		case CM_REQUESTING_COMMIT:		// 9901
					...
					if (last_data_msg != 0)
					{
						if ((last_data_msg == 3002) || (last_data_msg == 3008))
						{
							GerrLogMini (	GerrId,
											"%s: %d rows received, %d rows processed.",
											(last_data_msg == 3008) ? "Hospitalization - shouldn't be here" : "Lab Results",
